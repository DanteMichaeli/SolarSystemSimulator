        ...
        for line <- lines do
            if firstLine then
              ...
              if cols(0) == "sun" then try
                ...
                case nonPositiveRad if cols(2).toDouble <= 0 => throw new IllegalArgumentException("Non-positive radius error - Make sure none of your radii are negative or zero.")
                case nonPositiveMass if cols(3).toDouble <= 0 => throw new IllegalArgumentException("Non-positive mass error - Make sure none of your masses are negative or zero.")
                case negativeCoords if cols(4).toDouble < 0 || cols(5).toDouble < 0 => throw new IllegalArgumentException("Negative coordinates error - Make sure none of your initializing coordinates are negative.")

              else if cols(0) == "pla" then try
